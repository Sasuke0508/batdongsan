...
import { areaSizeOptions } from "../../constants";

function SelectAreaSize({ value, onChange, open, toggle }) {
    const label = useMemo(() => {
        if (!value.from && !value.to) {
            return "Diện tích";
        } else if (!value.from) {
            return `Dưới ${value.to} m2`;
        } else if (!value.to) {
            return `Trên ${value.from} m2`;
        } else {
            return `Từ ${value.from} đến ${value.to} m2`;
        }
    }, [value]);
    return (
        <div>
            <Dropdown className="search__price" isOpen={open} toggle={() => toggle("areaSize")} direction={"down"}>
